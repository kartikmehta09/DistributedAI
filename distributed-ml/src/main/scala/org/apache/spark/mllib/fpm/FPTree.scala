 ...
 * @tparam T item type
 */
private[fpm] class FPTree[T] extends Serializable with Logging {
  ...
  private def project(suffix: T): FPTree[T] = {
    ...
    if (summaries.contains(suffix)) {
      ...
      summary.nodes.foreach { node => //filter(_.isLeaf).leafNodes
        var t = List.empty[T]
        log.warn(s"判断${node.item}是否叶子结点${node.isLeaf}")
    ...
    //    tree.summaries.foreach(_._2.retainLeafNodes)
    tree
  }
  ...
  def extractTreeBranches(
      minCount: Long,
      validateSuffix: T => Boolean = _ => true): Iterator[(List[T], Long)] = {
    summaries
      .filter{case (item, summary) => validateSuffix(item) && !summary.leafNodes.isEmpty}
      .iterator.flatMap { case (item, summary) =>
            summary.leafNodes.map { node => 
              var t = List.empty[T]
//              log.warn(s"判断${node.item}是否叶子结点${node.isLeaf}")
              var curr = node //.parent
              while (!curr.isRoot) {
...
private[fpm] object FPTree extends Logging {
  ...
  private class Summary[T] extends Serializable {
    ...
    def leafNodes = nodes.filter(_.isLeaf)
    def retainLeafNodes = {
      nodes --= nodes.filterNot(_.isLeaf)
      nodes
    }
  }
}
