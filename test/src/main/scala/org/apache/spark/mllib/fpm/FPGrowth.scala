...
class FPGrowth private (
  ...
  def run[Item: ClassTag](data: RDD[Array[Item]]): FPGrowthModel[Item] = {
    ...
    val freqItemsets = genFreqCloseItemsets_V1(data, minCount, freqItems, partitioner)
//      genFreqItemsets(data, minCount, freqItems, partitioner)
  ...
  private def genFreqCloseItemsets_V1[Item: ClassTag](
      data: RDD[Array[Item]],
      minCount: Long,
      freqItems: Array[Item],
      partitioner: Partitioner): RDD[FreqItemset[Item]] = {
    val itemToRank = freqItems.zipWithIndex.toMap
    val a = data.flatMap { transaction =>
      genCondTransactions(transaction, itemToRank, partitioner)
    }.aggregateByKey(new FPTree[Int], partitioner.numPartitions)(
      (tree, transaction) => tree.add(transaction, 1L),
      (tree1, tree2) => tree1.merge(tree2))
    .flatMap { case (part, tree) =>
      tree.extractTreeBranches(minCount, x => partitioner.getPartition(x) == part)
    }
    a.map(f=> (1, f)).aggregateByKey(new FPTree[Int])(
     (tree, transaction) => tree.add(transaction._1, transaction._2), 
     (tree1, tree2) => tree1.merge(tree2))
    .flatMap { case (index, tree) =>
      tree.extractTreeBranches(0)
    }.map { case (ranks, count) =>
      new FreqItemset(ranks.map(i => freqItems(i)).toArray, count)
    } 
  }
